		...
		try {
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nome do bot* : ${me.name}\n*Número do Bot* : @${me.jid.split('@')[0]}\n*Prefixo* : ${prefix}\n*Número de contatos bloqueados* : ${blocked.length}\n*O bot etá ativo em* : ${kyun(uptime)}`
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falhou no momanto da conversão ${tipe} para figurinhas`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legenda ${prefix} adesivo ou tag de imagens que foram enviadas`)
					}
					break
				case 'gtts':
				...
				case 'ytsearch':
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Onde stá a url?')
				...
				case 'tiktokstalk':
					try {
						...
						reply('Possível nome de usuário')
					}
					break
				case 'nulis':
				case 'tulis':
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('Onde está a url?')
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('Onde etá o texto?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'clearall':
					...
					reply('Todos os bate-papos apagados com sucesso :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é você?')
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Rebaixado com sucesso @${mentioned[0].split('@')[0]} A membro do grupo!`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Quer adcionar um gênio?')
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adcionar destino, talvez porque é privado')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('A pessoa que você deseja expulsar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('O modo simi foi desativado desse grupo ✔️')
					} else {
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('O recurso de boas-vindas foi ativado nesse grupo ✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('O recurso de boas vindas foi desativado desse grupo ✔️')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
                                      break
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('A tag que você deseja clonar')
